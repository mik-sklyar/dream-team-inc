...
public class FindDuplicatePasswordsStrategy extends EmployeeOperationStrategy {
    ...
    private Map<String, List<String>> findDuplicatePasswordsTwoThreads() {
        ...
        executorService.submit(() -> processBatch(firstHalf, passwordMap));
        executorService.submit(() -> processBatch(secondHalf, passwordMap));
    ...
    private void processBatch(List<Employee> batch, ConcurrentHashMap<String, List<String>> passwordMap) {
        String threadName = Thread.currentThread().getName();
        System.out.println(threadName + ": обрабатывает " + batch.size() + " сотрудников");
